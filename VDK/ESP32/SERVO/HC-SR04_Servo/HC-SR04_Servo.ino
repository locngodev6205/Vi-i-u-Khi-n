...
// HC-SR04 & Servo
const int TRIG_PIN  = 21; //(Output)
const int ECHO_PIN  = 22; //(Input)
...
<body>
  <h2>ESP32 - CẢM BIẾN HC-SR04 & SERVO</h2>
  <hr>
  ...
  <script>
    // HC-SR04 phản hồi nhanh nên có thể cập nhật mỗi 500ms
    setInterval(() => {
      fetch('/status')
        .then(res => res.json())
        .then(data => {
...
void handleStatus() {
  ...
  // Gửi giá trị qua JSON với key "dist"
