...
<body>
  <h2>ESP32 DEVKIT V1 - DHT11 & STEPPER</h2>
  <hr>
  <p><b>Nhiệt độ:</b> <span id='tempValue'>0</span> &deg;C</p>
  <p><b>Độ ẩm:</b> <span id='humValue'>0</span> %</p>
  ...
  <script>
    // Cập nhật dữ liệu mỗi 2 giây (DHT11 đọc chậm, không nên fetch quá nhanh)
    setInterval(() => {
      fetch('/status')
        .then(res => res.json())
        .then(data => {
